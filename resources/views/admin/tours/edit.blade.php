...
@section('content')
    <div class="card card-primary">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form method="POST" autocomplete="off" action="{{ route('tours.update', [$tour->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" value="{{ $tour->departure_date }}" id="departure_dates" class="form-control"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="category_id">
                        ...
                        @foreach ($categories as $key => $val)
                            <option {{ $val->id == $tour->category_id ? 'selected' : '' }}
                                {{ $val->category_parent == 0 ? 'disabled' : '' }} value="{{ $val->id }}">
                                @php
                                    $str = '';
